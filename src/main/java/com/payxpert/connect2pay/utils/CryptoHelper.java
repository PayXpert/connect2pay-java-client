...
public class CryptoHelper {
  ...
  public static String sha512(String text) {
    ...
    if (text != null) {
      ...
      if (md != null) {
        try {
          result = Hex.encodeHexString(md.digest(text.getBytes(CHARSET_NAME)));
  ...
  public static byte[] encryptAES(String text, byte[] encryptionKey, byte[] encryptionIV) throws Exception {
    if (text != null) {
      byte[] data = null;
      try {
        data = text.getBytes(CHARSET_NAME);
      } catch (UnsupportedEncodingException e) {
        e.printStackTrace();
        return null;
      }
  ...
  public static String decryptAESBase64ToString(String text, byte[] encryptionKey, byte[] encryptionIV)
    ...
    return (decrypted != null) ? new String(decrypted, CHARSET_NAME) : null;
  }
